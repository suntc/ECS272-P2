...
class BubbleChart extends Component {
    ...
    componentDidMount() {
        let sentiDict = this.props.sentiDict;
        this.setState({prevsentiDict: sentiDict});
        this.renderGraph(sentiDict);
        autorun(() => this.activeCircle());
    }

    componentDidUpdate() {
        let prevsentiDict = this.state.prevsentiDict;
        let sentiDict = this.props.sentiDict;
        if (JSON.stringify(prevsentiDict) === JSON.stringify(sentiDict)) { // simple comparison
            return;
        }
    ...
    activeCircle = () => {
        ...
        .style("stroke", (d, i)=>{
            let chs = d.pair.split('-');
            console.log("chs", chs);
            if (this.props.dataStore.selectedChara.has(chs[0]) || this.props.dataStore.selectedChara.has(chs[1]))
                return "#db4c8e";
            let rpair = chs[1] + '-' + chs[0]
            if (this.props.dataStore.selectedLink.has(d.pair) || this.props.dataStore.selectedLink.has(rpair))
                return "#db4c8e";
        })
    ...
    renderGraph = (sentiDict) => {
        ...
        for(let i = 0; i < keys.length; ++i) {
            sentiDict[keys[i]].forEach((d) => {
                cvnodes.push({'idx': i, 'polarity': d.polarity, len: d.lines.length, line: d.lines, pair:keys[i]});
            })
        ...
        let clusterSt = -(height / 15);
        var simulation = d3.forceSimulation()
            .force("charge", d3.forceManyBody().strength(clusterSt))
        ...
        function draw(clusters) {
            ...
            let rScale = (len) => {
                let base = height / 120., maxs = height / 40;
                let sc = 2;
                let size = len * sc + base;
                if (size > maxs) return maxs;
                return size;
            }

            let colors = ['#005824','#1A693B','#347B53','#4F8D6B','#699F83','#83B09B','#9EC2B3','#B8D4CB','#D2E6E3','#EDF8FB','#FFFFFF','#F1EEF6','#E6D3E1','#DBB9CD','#D19EB9','#C684A4','#BB6990','#B14F7C','#A63467','#9B1A53','#91003F'];

            let circleSt = -(height / 25);

            var circlesimulation = d3.forceSimulation()
                .force("charge", d3.forceManyBody().strength(circleSt))
            ...
            var node = g.append("g")
                ...
                .data(cvnodes)
                .enter().append("circle")
                .attr("r", (d) => rScale(d.len))
                .classed("clickable", true)
                .attr("fill", function(d) { return colorScale(d.polarity) })
                .on("mouseover", (d, i) => {
                    dataStore.setActiveLine([d.line]);
                })
                .on("mouseout", (d, i) => {
                    dataStore.clearActiveLine();
                })
                .on("click", (d, i) => {
                    dataStore.setSelectedLine([d.line]);
                })
        ...
        // add legend
        let lcolor = ['#1A693B','#FFFFFF','#9B1A53'];
    ...
    render() {
        return(
            <div id="bb-contain" className="h-cell">
                <p className="tooltitle">Sentiment View (Conversations grouped by characters)</p>
